...
const shuffleArray = arr => arr.map(i => [Math.random(), i]).sort((a, b) => a[0] - b[0]).map(i => i[1]);
...
function generateMaze(width, height) {
  ...
  var start = vec2(0, 0);
  while (start.x % 2 == 0) { start.x = randomIntBetween(1, width - 1); }
  while (start.y % 2 == 0) { start.y = randomIntBetween(1, height - 1); }
  ...
  while (maze[entrance.x][1] == WALL) {
    entrance.x = randomIntBetween(1, width - 1);
  }
  ...
  while (maze[exit.x][height - 2] == WALL) {
    exit.x = randomIntBetween(1, width - 1);
  }
