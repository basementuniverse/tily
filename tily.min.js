...
return Tily.Cell.fromData(buffer,data);};return Cell;}());Tily.TileLayer=(function(){"use strict";function TileLayer(container){this.container=container;this.font="sans-serif";this.foreground="white";this.background="";this.opacity=1;this.clip=false;this.tiles=[];}
...
context.restore();};TileLayer.prototype.getData=function(){return{font:this.font,foreground:this.foreground,background:this.background,opacity:this.opacity,clip:this.clip,tiles:this.tiles};};TileLayer.fromData=function(container,data){const layer=new Tily.TileLayer(container);layer.font=data.font;layer.foreground=data.foreground;layer.background=data.background;layer.opacity=data.opacity;layer.clip=data.clip;layer.tiles=data.tiles;return layer;};return TileLayer;}());Tily.ActiveTileBase=(function(){"use strict";function ActiveTileBase(){this.layers=[];this.animations=[];this.font=null;this.foreground=null;this.opacity=null;this.offset=null;this.scale=null;this.rotation=null;}
...
context.restore();};ActiveTile.prototype.getData=function(){return{layers:this.layers.map(i=>i.getData()),position:this.position,zIndex:this.zIndex,clip:this.clip,wrap:this.wrap,flip:this.flip,font:this.font,foreground:this.foreground,opacity:this.opacity,offset:this.offset,scale:this.scale,rotation:this.rotation};};ActiveTile.fromData=function(data){const tile=new Tily.ActiveTile(data.position.x,data.position.y,data.zIndex);tile.layers=data.layers.map(i=>Tily.ActiveTileLayer.fromData(tile,tile,i));tile.clip=data.clip;tile.wrap=data.wrap;tile.flip=data.flip;tile.font=data.font;tile.foreground=data.foreground;tile.opacity=data.opacity;tile.offset=data.offset;tile.scale=data.scale;tile.rotation=data.rotation;return tile;};return ActiveTile;}(Tily.ActiveTileBase));Tily.ActiveTileLayer=(function(_super){"use strict";__extends(ActiveTileLayer,_super);function ActiveTileLayer(activeTile,parent){_super.call(this);this.activeTile=activeTile;this.parent=parent;this.text="";}
