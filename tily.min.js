...
return Tily.Cell.fromData(buffer,data);};return Cell;}());Tily.TileLayer=(function(){"use strict";function TileLayer(container){this.container=container;this.font="sans-serif";this.foreground="white";this.background="";this.opacity=1;this.compositeMode="source-over";this.clip=false;this.centered=false;this.tiles=[];}
...
context.fillText(this.text,p.x,p.y);if(this.inheritedOutline!==null){context.strokeText(this.text,p.x,p.y);}
for(let i=0,length=this.layers.length;i<length;i++){this.layers[i].draw(context,elapsedTime,tileSize);}
