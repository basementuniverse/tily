...
(function(){"use strict";var vec2=function(x,y){if(arguments.length==1){if(x instanceof Array&&x.length>1){return{x:x[0],y:x[1]};}else if(x.x!==undefined&&x.y!==undefined){return{x:x.x,y:x.y};}
return{x:0,y:0};}
...
return o;};}());var __extends=(this&&this.__extends)||function(d,b){for(var p in b){if(b.hasOwnProperty(p)){d[p]=b[p];}}
function __(){this.constructor=d;}
d.prototype=b===null?Object.create(b):(__.prototype=b.prototype,new __());};const Tily={};Tily.utility={clamp:function(a,min,max){if(min===undefined){min=0;};if(max===undefined){max=1;};return(a<min?min:(a>max?max:a));},lerp:function(a,b,i){return a*(1-i)+b*i;},colour:function(c){return`rgba(${Math.round(c.r)},${Math.round(c.g)},${Math.round(c.b)},${Math.round(c.a * 100) / 100})`;},outline:function(s){const parts=s.split(' ');return{width:parseFloat(parts[0])||0.1,colour:parts[1]||'transparent'};},shadow:function(s){const parts=s.split(' ');return{blur:parseFloat(parts[0]||1),xOffset:parseFloat(parts[1]||0),yOffset:parseFloat(parts[2]||0),colour:parts[3]};}};Tily.Main=(function(){"use strict";const _defaultTilyOptions={size:null,handleResize:true,showFPS:false,renderLoop:true,beforeDrawFunction:null,afterDrawFunction:null};function Main(canvas,options){if(!window){console.log("Couldn't find window. Tily.Main can only be created in the browser!");return}
...
this.finished=true;return this.finish;};Object.defineProperty(Transition.prototype,"amount",{get:function(){return Tily.utility.clamp(this.currentTime/this.totalTime);}});return Transition;}());Tily.BufferTransition=(function(_super){"use strict";__extends(BufferTransition,_super);function BufferTransition(start,finish,options){_super.call(this,start,finish,options);}
...
this.scale=Tily.utility.clamp(this.scale,Math.max(this.options.minimumScale,1),maximumScale);this.tileSize=(lockedAxis=="y"?height:width)/this.scale;this.viewSize.width=width/this.tileSize;this.viewSize.height=height/this.tileSize;if(this.options.clampCamera){const centerX=this.viewSize.width*0.5-0.5,centerY=this.viewSize.height*0.5-0.5;this.offset=offset=vec2(Tily.utility.clamp(offset.x,centerX,this.size.width-centerX-1),Tily.utility.clamp(offset.y,centerY,this.size.height-centerY-1));}
...
let c;if(this.centered===true){c=vec2.mul(vec2(0.5,0.5),tileSize);context.textAlign="center";context.textBaseline="middle";}else{c=vec2.mul(vec2(0,0),tileSize);context.textAlign="left";context.textBaseline="top";}
...
this.animations=this.animations.filter(i=>!i.finished);};return ActiveTileBase;}());Tily.ActiveTile=(function(_super){"use strict";__extends(ActiveTile,_super);function ActiveTile(x,y,zIndex){_super.call(this);this.position=vec2(x||0,y||0);this.zIndex=zIndex||0;this.clip=false;this.wrap=false;this.flip=false;this.font="sans-serif";this.fontStyle="normal";this.fontSize=null;this.foreground="white";this.outline=null;this.shadow=null;this.opacity=1;this.compositeMode="source-over";this.offset=vec2();this.scale=vec2(1,1);this.rotation=0;this.centered=false;this.destroyed=false;}
ActiveTile.prototype.move=function(direction,options){const directions={up:vec2(0,-1),down:vec2(0,1),left:vec2(-1,0),right:vec2(1,0)};this.position=vec2.add(this.position,directions[direction]);const animation=new Tily.OffsetAnimation(this,vec2.mul(directions[direction],-1),vec2(),options);this.animations.push(animation);return new Promise(function(resolve,reject){animation.finishedCallback=resolve;});};ActiveTile.prototype.addLayer=function(layer,z){layer=layer||new Tily.ActiveTileLayer(this);layer.activeTile=this;layer.parent=this;return _super.prototype.addLayer.call(this,layer,z);};function drawLayers(layers,context,elapsedTime,tileSize){for(let i=0,length=layers.length;i<length;i++){layers[i].draw(context,elapsedTime,tileSize);}}
...
let p;if(this.centered===true){p=vec2(0,0);context.textAlign="center";context.textBaseline="middle";}else{p=vec2(-tileSize*0.5,-tileSize*0.5);context.textAlign="left";context.textBaseline="top";}
...
if(this.finishedCallback&&!this.finished){this.finishedCallback(this.start,this.finish);}
this.finished=true;return 1;};return Animation;}(Tily.Transition));Tily.OffsetAnimation=(function(_super){"use strict";__extends(OffsetAnimation,_super);function OffsetAnimation(activeTile,start,finish,options){if(!options.easeFunction){options.easeFunction=(a,b,i)=>vec2(Tily.utility.lerp(a.x,b.x,i),Tily.utility.lerp(a.y,b.y,i));}
_super.call(this,activeTile,start,finish,options);}
OffsetAnimation.prototype.update=function(elapsedTime){const amount=_super.prototype.update.call(this,elapsedTime);this.activeTile.offset=this.easeFunction(this.start,this.finish,amount);};return OffsetAnimation;}(Tily.Animation));Tily.ScaleAnimation=(function(_super){"use strict";__extends(ScaleAnimation,_super);function ScaleAnimation(activeTile,start,finish,options){if(!options.easeFunction){options.easeFunction=(a,b,i)=>vec2(Tily.utility.lerp(a.x,b.x,i),Tily.utility.lerp(a.y,b.y,i));}
_super.call(this,activeTile,start,finish,options);}
ScaleAnimation.prototype.update=function(elapsedTime){const amount=_super.prototype.update.call(this,elapsedTime);this.activeTile.scale=this.easeFunction(this.start,this.finish,amount);};return ScaleAnimation;}(Tily.Animation));Tily.ForegroundAnimation=(function(_super){"use strict";__extends(ForegroundAnimation,_super);function ForegroundAnimation(activeTile,start,finish,options){if(!options.easeFunction){options.easeFunction=(a,b,i)=>({r:Tily.utility.lerp(a.r,b.r,i),g:Tily.utility.lerp(a.g,b.g,i),b:Tily.utility.lerp(a.b,b.b,i),a:Tily.utility.lerp(a.a,b.a,i)});}
_super.call(this,activeTile,start,finish,options);}
ForegroundAnimation.prototype.update=function(elapsedTime){const amount=_super.prototype.update.call(this,elapsedTime);this.activeTile.foreground=Tily.utility.colour(this.easeFunction(this.start,this.finish,amount));};return ForegroundAnimation;}(Tily.Animation));Tily.OpacityAnimation=(function(_super){"use strict";__extends(OpacityAnimation,_super);function OpacityAnimation(activeTile,start,finish,options){_super.call(this,activeTile,start,finish,options);}
...
this.activeTile.text=text;};return TextAnimation;}(Tily.Animation));Tily.OutlineAnimation=(function(_super){"use strict";__extends(OutlineAnimation,_super);function OutlineAnimation(activeTile,start,finish,options){if(!options.easeFunction){options.easeFunction=(a,b,i)=>{const ao=Tily.utility.outline(a);const bo=Tily.utility.outline(b);ao.colour=parseColor(ao.colour);bo.colour=parseColor(bo.colour);const result={width:Tily.utility.lerp(ao.width,bo.width,i),colour:Tily.utility.colour({r:Tily.utility.lerp(ao.colour.r,bo.colour.r,i),g:Tily.utility.lerp(ao.colour.g,bo.colour.g,i),b:Tily.utility.lerp(ao.colour.b,bo.colour.b,i),a:Tily.utility.lerp(ao.colour.a,bo.colour.a,i)})};return`${result.width} ${result.colour}`;};}
_super.call(this,activeTile,start,finish,options);}
OutlineAnimation.prototype.update=function(elapsedTime){const amount=_super.prototype.update.call(this,elapsedTime);this.activeTile.outline=this.easeFunction(this.start,this.finish,amount);};return OutlineAnimation;}(Tily.Animation));Tily.ShadowAnimation=(function(_super){"use strict";__extends(ShadowAnimation,_super);function ShadowAnimation(activeTile,start,finish,options){if(!options.easeFunction){options.easeFunction=(a,b,i)=>{const as=Tily.utility.shadow(a);const bs=Tily.utility.shadow(b);as.colour=parseColor(as.colour);bs.colour=parseColor(bs.colour);const result={blur:Tily.utility.lerp(as.blur,bs.blur,i),xOffset:Tily.utility.lerp(as.xOffset,bs.xOffset,i),yOffset:Tily.utility.lerp(as.yOffset,bs.yOffset,i),colour:Tily.utility.colour({r:Tily.utility.lerp(as.colour.r,bs.colour.r,i),g:Tily.utility.lerp(as.colour.g,bs.colour.g,i),b:Tily.utility.lerp(as.colour.b,bs.colour.b,i),a:Tily.utility.lerp(as.colour.a,bs.colour.a,i)})};return`${result.blur} ${result.xOffset} ${result.yOffset} ${result.colour}`;};}
_super.call(this,activeTile,start,finish,options);}
ShadowAnimation.prototype.update=function(elapsedTime){const amount=_super.prototype.update.call(this,elapsedTime);this.activeTile.shadow=this.easeFunction(this.start,this.finish,amount);};return ShadowAnimation;}(Tily.Animation));if(typeof module!=="undefined"){module.exports={Tily:Tily,vec2:vec2};}
