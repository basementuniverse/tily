...
this.animations=this.animations.filter(i=>!i.finished);};return ActiveTileBase;}());Tily.ActiveTile=(function(_super){"use strict";__extends(ActiveTile,_super);function ActiveTile(x,y,zIndex){_super.call(this);this.position=vec2(x||0,y||0);this.zIndex=zIndex||0;this.clip=false;this.wrap=false;this.flip=false;this.font="sans-serif";this.fontStyle="normal";this.fontSize=null;this.foreground="white";this.opacity=1;this.compositeMode="source-over";this.offset=vec2();this.scale=vec2(1,1);this.rotation=0;this.destroyed=false;}
...
context.restore();};ActiveTile.prototype.getData=function(){return{layers:this.layers.map(i=>i.getData()),position:this.position,zIndex:this.zIndex,clip:this.clip,wrap:this.wrap,flip:this.flip,font:this.font,fontStyle:this.fontStyle,fontSize:this.fontSize,foreground:this.foreground,opacity:this.opacity,compositeMode:this.compositeMode,offset:this.offset,scale:this.scale,rotation:this.rotation};};ActiveTile.fromData=function(data){const tile=new Tily.ActiveTile(data.position.x,data.position.y,data.zIndex);tile.layers=data.layers.map(i=>Tily.ActiveTileLayer.fromData(tile,tile,i));tile.clip=data.clip;tile.wrap=data.wrap;tile.flip=data.flip;tile.font=data.font;tile.fontStyle=data.fontStyle;tile.fontSize=data.fontSize;tile.foreground=data.foreground;tile.opacity=data.opacity;tile.compositeMode=data.compositeMode;tile.offset=data.offset;tile.scale=data.scale;tile.rotation=data.rotation;return tile;};return ActiveTile;}(Tily.ActiveTileBase));Tily.ActiveTileLayer=(function(_super){"use strict";__extends(ActiveTileLayer,_super);function ActiveTileLayer(activeTile,parent){_super.call(this);this.activeTile=activeTile;this.parent=parent;this.text="";}
